...
// InMemoryStore represents the session in-memory store.
...
func NewWithCleanupInterval(cleanupInterval time.Duration) *InMemoryStore {
	m := &InMemoryStore{
...
// be set to false.
func (m *InMemoryStore) Find(token string) ([]byte, bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// time are updated.
func (m *InMemoryStore) Commit(token string, b []byte, expiry time.Time) error {
	m.mu.Lock()
	m.items[token] = item{
		object:     b,
...
// instance.
func (m *InMemoryStore) Delete(token string) error {
	m.mu.Lock()
...
func (m *InMemoryStore) startCleanup(interval time.Duration) {
...
func (m *InMemoryStore) StopCleanup() {
	if m.stopCleanup != nil {
		m.stopCleanup <- true
	}
}

func (m *InMemoryStore) deleteExpired() {
